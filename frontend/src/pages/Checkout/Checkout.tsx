...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      ...
      if (ids.length === 0) { setProductsById({}); return; }
      ...
      try {
        ...
        for (const e of entries) { if (!e) continue; map[e.id] = e.product; }
        setProductsById(map);
      } finally {
        if (cancelled) return;
  ...
  async function onSubmit() {
    ...
    if (shippingMethod === 'delivery') {
      if (!shippingAddressLine1 || !shippingCity || !shippingProvince || !shippingPostalCode) {
    ...
    if (!name) { setError('Ingresá tu nombre completo para confirmar el pedido.'); return; }
    ...
    try {
      ...
      const createdTotal = Number(createdOrder?.total ?? subtotal);

      const orderState: OrderConfirmationState = {
        orderNumber,
        subtotal: Number.isFinite(subtotal) ? subtotal : summary.subtotal,
        shippingCost: 0,
        total: Number.isFinite(createdTotal) ? createdTotal : summary.subtotal,
        shippingMethod,
                  ...
                  {shippingMethod === 'delivery' ? (
                    <div className="checkout-deliveryGrid">
                      ...
                      <label className="checkout-field">
                        Código postal
                        <input
                          value={cart.shipping.postalCode}
